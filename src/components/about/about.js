...
const About = ({ showLightTheme, t }) => {
  ...
  const moveElements = (e) => {
    ...
    leftColumn.current.style.transform = `matrix3d(1,0,0.00,${
      distanceFromMiddleX * 0.0000006
    },0.00,1,0.00,0,0,0,1,0,${distanceFromMiddleX * -0.09},${
      distanceFromMiddleY * -0.09
    },0,1)`;
  ...
  return (
    ...
    >
      <div className="about-container" ref={aboutContainer}>
        <Waypoint onEnter={() => handleWaypointEnterLeft(leftColumn.current)}>
          <div className="about-column presentation" ref={leftColumn}>
            <p>
              <span className="intro">Hello</span>, {t("about.intro")}
            </p>
            <p>{t("about.first-paragraph")}</p>
            <p>{t("about.second-paragraph")}</p>
          </div>
        </Waypoint>
        <Waypoint onEnter={() => handleWaypointEnterRight(rightColumn.current)}>
          <div className="about-column skills" ref={rightColumn}>
            <h5>
              <span aria-label="emoji-computer" role="img">
                💻
              </span>
              {t("about.dev-title")}
            </h5>
            <ul>
              <li className="bolder">React.js (hooks) / Typescript / Nextjs</li>
              <li className="bolder">Redux / Redux Saga / Websockets</li>
            ...
            <h5>
              <span aria-label="emoji-worker" role="img">
                🎨
              </span>{" "}
              UI DESIGN
            </h5>
            <ul>
              <li className="bolder">Adobe / Zeplin</li>
            ...
            <ul>
              <li>{t("about.also-first")}</li>
              <li>{t("about.also-second")}</li>
            </ul>
          </div>
        </Waypoint>
      </div>
